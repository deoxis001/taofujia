...
class Sms {
     ...
     */
    public function send($mobile,$content,$log_type) {
       $shopwwi_sms_type=C('shopwwi_sms_type');
		if($shopwwi_sms_type==1)
		{
			return $this->mysend_smsbao($mobile,$content,$log_type);
		}
		if($shopwwi_sms_type==2)
		{
			return $this->mysend_yunpian($mobile,$content,$log_type);
		}
		if($shopwwi_sms_type==3)
		{
			return $this->mysend_alidayu($mobile,$content,$log_type);
		}
    }
...
/*
	...
	*/
	...
	http://api.smsbao.com/sms?u=USERNAME&p=PASSWORD&m=PHONE&c=CONTENT
	*/
	private function mysend_alidayu($mobile,$content,$log_type){

		$alidayu='alidayu';
		$plugin = str_replace('\\', '', str_replace('/', '', str_replace('.', '',$alidayu)));
		if (!empty($plugin)) {
			define('PLUGIN_ROOT', BASE_DATA_PATH . DS .'api/smsapi');
			require_once(PLUGIN_ROOT . DS . $plugin . DS . 'Send.php');
			return send_sms($mobile, $content,$log_type);
		}
		else
		{
			return false;
