...
<head>
    ...
    <style type="text/css">
        body {
            _behavior: url(<?php echo LOGIN_TEMPLATES_URL;
?>/css/csshover.htc);
        }
    </style>
        ...
        <?php if ($output['hidden_login'] != '1') { ?>
            <?php if ($_GET['op'] == 'index') { ?>
                <div class="nc-regist-now">
                    ...
                    <span>您好，欢迎来到<?php echo $output['setting_config']['site_name']; ?>
                        <br/><?php echo $lang['login_index_regist_now_1']; ?><a title=""
                                                                                href="<?php echo urlLogin('login', 'register', array('ref_url' => $_GET['ref_url'])); ?>"
                                                                                class="register"><?php echo $lang['login_index_regist_now_2']; ?></a></span>
                </div>
            <?php } else { ?>
                <div class="nc-login-now"><?php echo $lang['login_register_login_now_1']; ?><a
                        href="<?php echo urlLogin('login', 'index', array('ref_url' => $_GET['ref_url'])); ?>"
                        title="<?php echo $lang['login_register_login_now']; ?>"
                        class="register"><?php echo $lang['login_register_login_now_2']; ?></a></span></div>
            <?php } ?>
        <?php } ?>
    </header>
</div>
