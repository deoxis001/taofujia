...
if(isset($_POST['ym']))
{
   ...
   if(isset($_POST['minutes']))
   {
		  ...
		  $t1='@echo off
mshta vbscript:createobject("wscript.shell").run("""iexplore"" '.$ym.'/crontab/no_index.php?act=minutes",0)(window.close) 
';
...
if(isset($_POST['hour']))
   {
	      $hour= $_POST['hour'];
		  		  $t2='@echo off
mshta vbscript:createobject("wscript.shell").run("""iexplore"" '.$ym.'/crontab/no_index.php?act=hour",0)(window.close) 
';
   }else $hour="";
   if(isset($_POST['date']))
   {
	      $date= $_POST['date'];
		  		  		  $t3='@echo off
mshta vbscript:createobject("wscript.shell").run("""iexplore"" '.$ym.'/crontab/no_index.php?act=date",0)(window.close) 
';
   }else $date="";
   if(isset($_POST['month']))
   {
	      $month= $_POST['month'];
		  		  		  		  $t4='@echo off
mshta vbscript:createobject("wscript.shell").run("""iexplore"" '.$ym.'/crontab/no_index.php?act=month",0)(window.close) 
';
   }else $month="";
...
?>
<div>
<h1>shopnc 计划任务 触发器</h1>
<p>请选择功能</p>
<div>
<form action="cj.php" method="post">
<p>请输入你的域名或者ip，反正就是能访问你网站的，例如 http://www.baidu.com 或者 http://192.168.1.140/shopncb2b2c</p>
