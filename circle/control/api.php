...
class apiControl extends BaseCircleControl{
    ...
    public function get_theme_listOp() {
        ...
        $model = Model();
        $theme_list = $model->table('circle_theme')->field('circle_theme.*, is_recommend*rand()*10000 + has_affix*rand() as rand')->where(array( 'is_closed'=>0))->where(array('has_affix'=>1))->order('rand desc')->limit($data_count)->select();
        if(!empty($theme_list)){
